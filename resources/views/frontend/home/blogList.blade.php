<div class="text-red-950 py-16 px-4 text-center">
    <div class="text-center mb-12">
        <h2 class="text-5xl font-bold mb-4">Latest Blogs</h2>
        <p class="text-xl w-full md:w-1/2 mx-auto">
            Stay updated with our latest insights, tips, and tutorials from the world of blogging.
        </p>
    </div>

    <div class="w-full p-20 h-full flex flex-wrap justify-center items-center gap-20">

        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>
        <!-- Blog Card -->
        <div
            class="w-[400px] h-full bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition duration-300">
            <img src="assets\user\img\blogThumb.jpg" alt="Blog Image" class="w-full object-cover">
            <div class="p-6 flex flex-col justify-between h-full">
                <h3 class="text-2xl font-semibold mb-2">The Power of Writing Consistently</h3>
                <p class="text-red-800 mb-4">
                    Discover how maintaining a writing habit can boost your creativity and improve engagement.
                </p>
                <a href="#" class="text-red-950 font-semibold hover:underline">Read More →</a>
            </div>
        </div>





    </div>

    <button class="bg-red-950 text-lg text-red-200 rounded-xl py-4 px-10">Show All</button>
</div>
